...
- [x] In noiseless channel if error percentage is less than 20(which can test with **big_test.py** that loop though 100time simulation and find the maximum error percentage) there is no **eve.py** active, so **autocheck.py** will fail if eve is there, but can also see if same_basis_code is correct or not, and see the big difference with pair and unpair to analysis.(because we want to know if eve is there!)  
